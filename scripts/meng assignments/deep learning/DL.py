...
#        curx = np.zeros((1,3*statemem))
#        cury = np.zeros((1,1))
#        curx[0,:statemem] = leadv
#        curx[0,statemem:2*statemem] = vehv
#        curx[0,2*statemem:3*statemem] = hd
#        cury[0,0] = curmeas[j,2]
#        
#        #using np.append is extremely slow because you have to copy the entire array each call. 
#        #would be faster if you used regular python lists, but there are problems with converting things into tensors
#        if train_or_test[count]:
#            xtrain = np.append(xtrain, curx,axis=0)
#            ytrain = np.append(ytrain, cury,axis=0)
#        else:
#            xtest = np.append(xtest, curx,axis=0)
#            ytest = np.append(ytest, cury,axis=0)
        curx = list(leadv)
...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        self.out = kls.Dense(1, activation = 'tanh')
...
loss_fn = tf.keras.metrics.MeanSquaredError(name='train_test_loss')
